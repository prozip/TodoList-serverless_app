...
$(".add-tsk").click(function () {
  ...
  function add(id, todayy, className){
    $str = '<div class="todo-item all-list" id="' + id + '">' +
  ...
  if (initCheck){
    ...
    pywebview.api.addItem(encodeB64($_task), encodeB64($_taskDescriptionText),encodeB64($_taskDescriptionInnerHTML), 
      encodeB64($_textDelta), today, 'todo-item all-list').then(function (response) {
...
// export data
function exportData() {
  var itemList = $('.todo-box').find('.todo-item');
  console.log(itemList.map((index, item) => {
    return item
  }));
}